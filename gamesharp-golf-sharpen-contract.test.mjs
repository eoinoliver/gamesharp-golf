...
assert(/class="gss-journey-visual" data-gss-progress/.test(html),'interactive illustrated journey entry missing');
assert(/\.gss-journey-visual\[data-preview="prepare"\][\s\S]*\.gss-journey-visual\[data-preview="reflect"\]/.test(html),'entry illustration does not respond to all three intentions');
assert(/function journeyVisual\([\s\S]{0,500}data-moment/.test(html),'authoritative journey visual renderer missing');
...
assert(/journeyVisual\(rid,r\.label\)/.test(html),'illustration missing from symptom selection');
assert(/journeyVisual\(state\.regionId,region\.label\)/.test(html),'illustration missing from distinction');
assert(/journeyVisual\(r\.regionId,reg\.label\)/.test(html),'illustration missing from focus result');
