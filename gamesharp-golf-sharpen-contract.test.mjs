...
assert(/const CACHE = 'gamesharp-golf-v20'/.test(sw),'release cache version must evict the repeated-marker Sharpen shell');
...
assert(/const zoneVisualMarkup=moment=>/.test(html),'selected Sharpen moment lacks an authoritative visual renderer');
...
assert(/if\(stage!=='entry'\)cinemaEl\.insertAdjacentHTML\('beforeend',zoneVisualMarkup\(moment\)\)/.test(html),'selected moment does not render its synchronized visual');
assert(!/gss-zone-pulse|gssZoneBreathe/.test(html),'retired five-circle animation remains implemented');
assert(/@media\(prefers-reduced-motion:reduce\)\{\.gss-shot-line\{animation:none/.test(html),'reduced motion does not preserve shot information');
