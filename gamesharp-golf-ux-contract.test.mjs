...
assert.equal((html.match(/<button class="mode-card"/g)||[]).length,2,'home must expose exactly two direct play choices');
assert(/<details class="home-more">[\s\S]{0,500}PLAY A 3-HOLE DECISION RUN[\s\S]{0,300}PLAY A 9-HOLE DECISION RUN/.test(html),'secondary play formats must use progressive disclosure');
...
console.log(JSON.stringify({ok:true,homePrimaryActions:3,playChoices:2,secondaryPlayFormats:2,semanticNavButtons:9},null,2));
